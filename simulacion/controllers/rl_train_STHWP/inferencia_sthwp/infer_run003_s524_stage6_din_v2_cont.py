"""
Inferencia ciclo completo dinámico v2_cont — run003 seed=524 — checkpoint pico.

Modelo continuado 3M steps desde din_v2_final (48 dims).
Carga: checkpoint en paso 2501472 (pico de reward/éxito — 234 reward, 46% éxito ult100).
El modelo final (4M) regresa a 71 reward por exploración excesiva; el checkpoint del pico
es el mejor modelo de este seed.
100 ep × 28 goals = 2800 ciclos. World: warehouse_1.wbt (PEDESTRIAN_1 + PEDESTRIAN_2).
...
Salida: inferencia_sthwp/resultados/infer_run003_s524_stage6_din_v2_cont.csv
"""
...
MODEL_PATH = os.path.join(CONTROLLER_DIR, "pruebas",
                          "checkpoints_run003_s524_stage6_din_v2_cont",
                          "run003_s524_stage6_din_v2_cont_2501472_steps")
RUN_ID     = "run003_s524_din_v2_cont"
N_POR_GOAL = 100
OUT_DIR    = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH   = os.path.join(OUT_DIR, "infer_run003_s524_stage6_din_v2_cont.csv")
...
print(f"\n{'='*60}")
print(f" Inferencia DINÁMICA v2_cont (checkpoint pico) | {RUN_ID}")
print(f" Modelo: {os.path.basename(MODEL_PATH)} (paso 2501472 — pico)")
