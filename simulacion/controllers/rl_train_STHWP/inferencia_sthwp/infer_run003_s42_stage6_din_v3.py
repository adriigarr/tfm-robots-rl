"""
Inferencia ciclo completo dinámico v3 — run003 seed=42 — recompensa B1.

Checkpoint seleccionado: paso 4,101,792 (pico de tasa_exito durante entrenamiento;
el modelo final colapsa a 12.5% vs 25% en el pico).
100 ep × 28 goals = 2800 ciclos. World: warehouse_1.wbt (PEDESTRIAN_1 + PEDESTRIAN_2).

Salida: inferencia_sthwp/resultados/infer_run003_s42_stage6_din_v3.csv
"""
...
RUN_ID     = "run003_s42_din_v3"
N_POR_GOAL = 100
OUT_DIR    = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH   = os.path.join(OUT_DIR, "infer_run003_s42_stage6_din_v3.csv")
...
print(f" Modelo: {os.path.basename(MODEL_PATH)} (ckpt pico 4.1M)")
