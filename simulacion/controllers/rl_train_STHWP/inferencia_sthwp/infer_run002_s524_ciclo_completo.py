"""
Inferencia ciclo completo — run002 seed=524 — INICIO → ESTANTERÍA → DESCARGA → INICIO.

Encadena dos modelos en un mismo episodio de Webots:
  1. stage4v2_final (approach + exit): zona_espera → goal → zona_descarga
  2. stage5_final   (retorno puro):    zona_descarga → zona_espera

100 episodios por goal × 28 goals = 2800 ciclos encadenados.

Clasificación del resultado:
  exito          — ciclo completo completado (las 3 fases)
  col_approach   — colisión durante approach (antes de llegar a estantería)
  col_exit       — colisión durante exit (llegó a estantería, falló en exit)
  col_retorno    — colisión durante retorno (llegó a descarga, falló en retorno)
  truncado_ap_ex — timeout en approach o exit
  truncado_ret   — timeout en retorno

Salida: inferencia_sthwp/resultados/infer_run002_s524_ciclo_completo.csv
"""
...
MODEL_S4_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "pruebas", "run002_s524_stage4v2_final")
MODEL_S5_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "pruebas", "run002_s524_stage5_final")
RUN_ID        = "run002_s524"
HEADING_SIGMA = 0.25
N_POR_GOAL    = 100
OUT_DIR       = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH      = os.path.join(OUT_DIR, "infer_run002_s524_ciclo_completo.csv")

# Entorno único — el stage se cambia entre la fase ap+exit y la fase retorno
...
print(f"\n{'='*60}")
print(f" Inferencia CICLO COMPLETO | {RUN_ID}")
...
for goal_idx in range(N_GOALS):
    ...
    for ep in range(1, N_POR_GOAL + 1):
        ...
        env.stage              = 4
        env._max_steps         = 4000    # goals 26/27 tienen paths largos
        env.PROB_APPROACH_S4   = 1.0     # siempre ciclo completo en inferencia
        ...
        llego_desc = exito_s4   # approach+exit exitoso = robot en zona_descarga
        ...
        print(f"    ep {ep:>3}/{N_POR_GOAL}  {resultado:<20}  "
...
print(f"\n{'='*60}")
print(f" RESUMEN — {N_TOTAL} ciclos deterministas | {RUN_ID}")
print(f"{'='*60}")
