"""
Inferencia dinámica v3 + REPLANIFICACIÓN — STH-WP s123.

Checkpoint: run003_s123_stage6_din_v3_2601472_steps
Baseline v3 sin replanning: 38.0%

Salida: inferencia_sthwp/resultados/infer_run003_s123_stage6_din_v3_replanning.csv
"""
...
MODEL_PATH = os.path.join(CONTROLLER_DIR, "pruebas",
                          "checkpoints_run003_s123_stage6_din_v3",
                          "run003_s123_stage6_din_v3_2601472_steps")
RUN_ID   = "run003_s123_din_v3_replanning"
N_POR_GOAL = 100
OUT_DIR  = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH = os.path.join(OUT_DIR, "infer_run003_s123_stage6_din_v3_replanning.csv")
...
print(f" Modelo: {os.path.basename(MODEL_PATH)} (ckpt pico)")
    ...
    for ep in range(1, N_POR_GOAL + 1):
        ...
        while not done and not truncated:
            ...
            ped_pos  = [(n.getField("translation").getSFVec3f()[0],
                         n.getField("translation").getSFVec3f()[1]) for n in ped_nodes]
            if replan_cooldown == 0 and pedestrian_bloquea(rx, ry, sx, sy, ped_pos):
                if replanificar(env, ped_pos):
...
print(f"\n  COMPARATIVA:")
print(f"    v3 sin replanning (baseline):  38.0%")
