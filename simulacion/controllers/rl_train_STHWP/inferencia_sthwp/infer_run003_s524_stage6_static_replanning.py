"""
Inferencia v1 estático + REPLANIFICACIÓN — STH-WP s524, peatones congelados.

Modelo: run003_s524_stage6_final (v1, ped_obs=False, 40 dims).
Peatones: físicamente presentes pero congelados en cada step.
Replanning: activo — detecta peatones en el segmento robot→subgoal y replaneamos.

Sin replanning: 0.7% (§23) — A* ignora peatones, subgoals caen sobre ellos.
Con replanning: ??? — A* temporal con peatón inflado evita el bloqueo.

Este script aísla el efecto del replanning SOLO, sin ningún entrenamiento
dinámico de por medio. Si alcanza ~100%, replanning solo basta; si no,
se necesita también entrenamiento con peatones.

Salida: inferencia_sthwp/resultados/infer_run003_s524_stage6_static_replanning.csv
"""
...
RUN_ID     = "run003_s524_static_replanning"
N_POR_GOAL = 100
OUT_DIR    = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH   = os.path.join(OUT_DIR, "infer_run003_s524_stage6_static_replanning.csv")
...
print(f"[INFO] {len(ped_nodes)} peatón(es) en el mundo (congelados en STATIC_POSITIONS)")

# Posiciones fijas (no cambian) — usamos las estáticas para replanning y freeze
...
print(f" Peatones: congelados en posiciones fijas")
