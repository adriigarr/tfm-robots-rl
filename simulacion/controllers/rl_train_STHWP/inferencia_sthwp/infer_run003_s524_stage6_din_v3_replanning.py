"""
Inferencia dinámica v3 + REPLANIFICACIÓN — STH-WP s524.

Igual que infer_run003_s524_stage6_din_v3.py pero con replanificación dinámica:
cuando un peatón se proyecta a menos de DIST_PERP_THRESHOLD metros del segmento
robot→subgoal, se añade temporalmente a la cuadrícula A* con inflado de
INFLATE_CELLS celdas y se recalcula la ruta desde la posición actual del robot
hasta el destino de la fase en curso. Cooldown de REPLAN_COOLDOWN steps tras
cada replanificación para evitar oscilación.

Baseline: v3 sin replanning → 45.4%
Pregunta: ¿el replanning supera ese techo?

Salida: inferencia_sthwp/resultados/infer_run003_s524_stage6_din_v3_replanning.csv
"""
...
RUN_ID     = "run003_s524_din_v3_replanning"
N_POR_GOAL = 100
OUT_DIR    = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH   = os.path.join(OUT_DIR, "infer_run003_s524_stage6_din_v3_replanning.csv")

DIST_PERP_THRESHOLD = 0.6   # m — radio cuerpo peatón (~0.4) + margen seguridad
INFLATE_CELLS       = 3     # celdas a bloquear alrededor del peatón (3 × 0.25m = 0.75m)
...
# Construir grid_nav una sola vez — se copia por episodio para no modificar el original
grid_nav, map_info = build_grid(MAP_PATH, margin=1.2)
ORIGEN     = map_info["origin"]
RESOLUTION = map_info["resolution"]
ROWS, COLS = grid_nav.shape

# Nodos de peatón (accesibles aunque ped_obs=True ya los tiene, los obtenemos manualmente
# para poder leer la posición en cada step sin depender de env._ped_nodes)
...
def replanificar(env, ped_positions):
    """
    Añade los peatones (inflados) a una copia temporal de grid_nav y recalcula
    la ruta A* desde la posición actual del robot hasta el final de env.full_path.
    Actualiza env.full_path si se encuentra ruta. Returns True en éxito.
    """
    ...
    path_world[-1] = goal_pos  # conservar el destino exacto de la fase
...
print(f"\n{'='*60}")
print(f" Inferencia DINÁMICA v3 + REPLANNING | {RUN_ID}")
print(f" Modelo: {os.path.basename(MODEL_PATH)} (ckpt pico 2.8M, ped_obs=True)")
...
for goal_idx in range(N_GOALS):
    ...
    for ep in range(1, N_POR_GOAL + 1):
        ...
        while not done and not truncated:
            # Posición actual del robot y subgoal de la política
            pos = env.robot_node.getField("translation").getSFVec3f()
            rx, ry = pos[0], pos[1]
            d_actual = env.D_AHEAD_ESCAPE if env._fase_escape else env.D_AHEAD_NORMAL
            subgoal  = compute_sth_subgoal(env.full_path, (rx, ry), d_actual)
            sx, sy   = subgoal

            # Posiciones actuales de peatones
            ped_pos = [(n.getField("translation").getSFVec3f()[0],
                        n.getField("translation").getSFVec3f()[1])
                       for n in ped_nodes]

            # Replanificación si peatón intercepta segmento robot→subgoal
...
print(f"\n  COMPARATIVA:")
print(f"    v3 sin replanning (baseline):   45.4%")
