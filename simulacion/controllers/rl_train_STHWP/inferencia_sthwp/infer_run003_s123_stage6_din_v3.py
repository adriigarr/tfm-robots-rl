"""
Inferencia ciclo completo dinámico v3 — run003 seed=123 — recompensa B1.

Checkpoint seleccionado: paso 2,601,472 (pico de tasa_exito 33.6% en step 2,612,064;
el modelo final colapsa a 9%). Partía del checkpoint pico v2_cont (2.5M).
100 ep × 28 goals = 2800 ciclos. World: warehouse_1.wbt (PEDESTRIAN_1 + PEDESTRIAN_2).

Salida: inferencia_sthwp/resultados/infer_run003_s123_stage6_din_v3.csv
"""
...
RUN_ID     = "run003_s123_din_v3"
N_POR_GOAL = 100
OUT_DIR    = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH   = os.path.join(OUT_DIR, "infer_run003_s123_stage6_din_v3.csv")
...
print(f" Modelo: {os.path.basename(MODEL_PATH)} (ckpt pico 2.6M)")
