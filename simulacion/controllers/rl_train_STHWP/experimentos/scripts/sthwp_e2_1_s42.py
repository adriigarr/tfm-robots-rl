"""
EXPERIMENTO E2.1 — Observación realista (solo LIDAR 5m, sin supervisor) | STHWP | SEED=42
...
Base: run003_s42_stage6_final (40 dims, stage 6 sin peatón)
"""
...
LOAD_PATH = os.path.join(CONTROLLER_DIR, "pruebas", "run003_s42_stage6_final")
SAVE_PATH = os.path.join(CONTROLLER_DIR, "pruebas", "sthwp_e2_1_s42_final")
CKPT_DIR  = os.path.join(CONTROLLER_DIR, "pruebas", "checkpoints_sthwp_e2_1_s42")

SEED = 42
random.seed(SEED); np.random.seed(SEED); torch.manual_seed(SEED)

print(f"\n[E2.1 STHWP s42] Obs realista: solo LIDAR 5m (sin supervisor)")
...
model.learn(
    ...
    callback            = [
        StatsCallback(goal_ids=env.goal_ids, stage=6, run_id="sthwp_e2_1_s42"),
        CheckpointCallback(save_freq=200_000, save_path=CKPT_DIR,
                           name_prefix="sthwp_e2_1_s42"),
    ],
    tb_log_name         = "sthwp_e2_1_s42",
    reset_num_timesteps = True,
)

model.save(SAVE_PATH)
print(f"[E2.1 STHWP s42] Guardado en {SAVE_PATH}.zip")
