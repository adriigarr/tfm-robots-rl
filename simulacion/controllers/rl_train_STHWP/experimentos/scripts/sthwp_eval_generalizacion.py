"""
EVALUACIÓN DE GENERALIZACIÓN | STHWP
...
Parametrizado por variables de entorno (fijadas por el runner):
  ...
  EVAL_MAX_STEPS  — límite de pasos por episodio (por defecto 7000)
...
RUN_ID     = f"sthwp_eval_gen_wh{WH}_s{SEED}" + ("_ped" if PED else "")

os.makedirs(os.path.dirname(CSV_PATH), exist_ok=True)

env = WebotsEnv(map_path=MAP_PATH, stage=6, ped_obs=PED)
env._max_steps = int(os.environ.get("EVAL_MAX_STEPS", 7000))
if PED:
        ...
        with open(CSV_PATH, "a", newline="") as f:
            csv.DictWriter(f, fieldnames=FIELDS).writerow(
                {"episodio": ep_global, "goal_id": goal_id, "rep": rep, "resultado": resultado,
                 ...
                 "warehouse_id": WH, "metodo": "sthwp", "seed": SEED, "peaton": int(PED)})
