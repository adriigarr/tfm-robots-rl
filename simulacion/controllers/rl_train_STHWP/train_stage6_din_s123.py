"""
ETAPA 6 DIN — Ciclo completo con peatones (fine-tuning dinámico).  1M steps  |  SEED=123

Requiere: pruebas/run003_s123_stage6_final.zip

Fine-tuning del modelo estático con obstáculos dinámicos activos (PEDESTRIAN_1 + PEDESTRIAN_2).
World: warehouse_1.wbt (con peatones).
LR reducida (5e-5) para preservar el conocimiento estático.

Salida: pruebas/run003_s123_stage6_din_final.zip
"""
...
model = PPO.load(
    f"./pruebas/{LOAD_ID}_stage6_final",
    env             = env,
    learning_rate   = 5e-5,
    ent_coef        = 0.01,
    seed            = SEED,
...
SAVE_PATH = f"./pruebas/{RUN_ID}_stage6_din_final"
CKPT_DIR  = f"./pruebas/checkpoints_{RUN_ID}_stage6_din"
...
model.learn(
    ...
    callback        = [
        ...
        CheckpointCallback(
            save_freq   = 50_000,
            save_path   = CKPT_DIR,
            name_prefix = f"{RUN_ID}_stage6_din",
        ),
    ],
    tb_log_name         = f"stage6_din_{RUN_ID}",
