"""
EXPERIMENTO E2.2b — Replanning LIDAR dinámico (umbral corregido) | SUBWP | SEED=42

Corrección respecto a E2.2:
  - LIDAR_MIN_DYNAMIC: 1.5m → 2.5m
  - Elimina falsos positivos de paredes en pasillos estrechos (~2m ancho).

Base: subwp_e2_1_s42_final (E2.1, no E2.2 que falló)
"""
...
SAVE_PATH = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_e2_2b_s42_final")
CKPT_DIR  = os.path.join(CONTROLLER_DIR, "pruebas", "checkpoints_subwp_e2_2b_s42")

SEED = 42
random.seed(SEED); np.random.seed(SEED); torch.manual_seed(SEED)

print(f"\n[E2.2b SUBWP s42] Replanning LIDAR dinámico (LIDAR_MIN_DYNAMIC=2.5m)")
...
model.learn(
    ...
    callback            = [
        StatsCallback(goal_ids=env.goal_ids, stage=6, run_id="subwp_e2_2b_s42"),
        CheckpointCallback(save_freq=200_000, save_path=CKPT_DIR,
                           name_prefix="subwp_e2_2b_s42"),
    ],
    tb_log_name         = "subwp_e2_2b_s42",
    reset_num_timesteps = True,
)

model.save(SAVE_PATH)
print(f"[E2.2b SUBWP s42] Guardado en {SAVE_PATH}.zip")
