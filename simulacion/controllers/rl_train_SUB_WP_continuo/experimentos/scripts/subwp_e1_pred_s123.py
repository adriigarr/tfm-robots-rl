"""
EXPERIMENTO E1_pred — Observación predictiva P1 | SUB-WP | SEED=123

Añade 4 dims de predicción de posición de P1 en t+1s y t+2s a la observación
(48→52 dims). Se parte de los pesos de E1 (s123) mediante weight transplant:
  ...
  - Primera capa (Linear 48→64): se extiende a Linear 52→64 inicializando
    las 4 nuevas columnas a cero.

Base: subwp_e1_1ped_s123_final (48 dims, 5M steps)
World: warehouse_1_subwp_1ped.wbt (solo PEDESTRIAN_1 activo)
...
SAVE_PATH  = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_e1_pred_s123_final")
CKPT_DIR   = os.path.join(CONTROLLER_DIR, "pruebas", "checkpoints_subwp_e1_pred_s123")
...
print(f"\n[E1_pred SUBWP s123] Weight transplant {OLD_OBS}→{NEW_OBS} dims")
...
# Continuar el contador de pasos del modelo base para que TensorBoard no empiece en 0
new_model.num_timesteps = old_model.num_timesteps

print(f"  Transplante completado. Continuando desde step {new_model.num_timesteps}...")

new_model.learn(
    total_timesteps     = 3_000_000,
    callback            = [
        StatsCallback(goal_ids=env.goal_ids, stage=6, run_id="subwp_e1_pred_s123"),
        CheckpointCallback(save_freq=100_000, save_path=CKPT_DIR,
                           name_prefix="subwp_e1_pred_s123"),
    ],
    tb_log_name         = "subwp_e1_pred_s123",
    reset_num_timesteps = False,
)

new_model.save(SAVE_PATH)
print(f"[E1_pred SUBWP s123] Guardado en {SAVE_PATH}.zip")
