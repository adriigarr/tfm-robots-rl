"""
EXPERIMENTO E2.2b — Replanning LIDAR dinámico (umbral corregido) | SUBWP | SEED=524

Corrección: LIDAR_MIN_DYNAMIC 1.5m → 2.5m para eliminar falsos positivos de paredes.
Base: subwp_e2_1_s524_final (E2.1)
"""
...
LOAD_PATH = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_e2_1_s524_final")
SAVE_PATH = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_e2_2b_s524_final")
CKPT_DIR  = os.path.join(CONTROLLER_DIR, "pruebas", "checkpoints_subwp_e2_2b_s524")

SEED = 524
random.seed(SEED); np.random.seed(SEED); torch.manual_seed(SEED)

print(f"\n[E2.2b SUBWP s524] Replanning LIDAR dinámico (LIDAR_MIN_DYNAMIC=2.5m)")
...
model.learn(
    ...
    callback            = [
        StatsCallback(goal_ids=env.goal_ids, stage=6, run_id="subwp_e2_2b_s524"),
        CheckpointCallback(save_freq=200_000, save_path=CKPT_DIR,
                           name_prefix="subwp_e2_2b_s524"),
    ],
    tb_log_name         = "subwp_e2_2b_s524",
    reset_num_timesteps = True,
)

model.save(SAVE_PATH)
print(f"[E2.2b SUBWP s524] Guardado en {SAVE_PATH}.zip")
