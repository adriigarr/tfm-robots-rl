"""
EXPERIMENTO E2.1 — Observación realista (solo LIDAR 5m, sin supervisor) | SUBWP | SEED=524
...
LOAD_PATH = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_e2_0_s524_final")
SAVE_PATH = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_e2_1_s524_final")
CKPT_DIR  = os.path.join(CONTROLLER_DIR, "pruebas", "checkpoints_subwp_e2_1_s524")

SEED = 524
random.seed(SEED); np.random.seed(SEED); torch.manual_seed(SEED)

print(f"\n[E2.1 SUBWP s524] Obs realista: solo LIDAR 5m (sin supervisor)")
...
model.learn(
    ...
    callback            = [
        StatsCallback(goal_ids=env.goal_ids, stage=6, run_id="subwp_e2_1_s524"),
        CheckpointCallback(save_freq=200_000, save_path=CKPT_DIR,
                           name_prefix="subwp_e2_1_s524"),
    ],
    tb_log_name         = "subwp_e2_1_s524",
    reset_num_timesteps = True,
)

model.save(SAVE_PATH)
print(f"[E2.1 SUBWP s524] Guardado en {SAVE_PATH}.zip")
