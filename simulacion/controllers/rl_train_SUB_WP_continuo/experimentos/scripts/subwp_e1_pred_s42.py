"""
EXPERIMENTO E1_pred — Observación predictiva P1 | SUB-WP | SEED=42
...
Base: subwp_e1_1ped_s42_final (48 dims, 5M steps)
...
LOAD_PATH  = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_e1_1ped_s42_final")
SAVE_PATH  = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_e1_pred_s42_final")
CKPT_DIR   = os.path.join(CONTROLLER_DIR, "pruebas", "checkpoints_subwp_e1_pred_s42")
...
print(f"\n[E1_pred SUBWP s42] Weight transplant {OLD_OBS}→{NEW_OBS} dims")
...
new_model.learn(
    ...
    callback            = [
        StatsCallback(goal_ids=env.goal_ids, stage=6, run_id="subwp_e1_pred_s42"),
        CheckpointCallback(save_freq=100_000, save_path=CKPT_DIR,
                           name_prefix="subwp_e1_pred_s42"),
    ],
    tb_log_name         = "subwp_e1_pred_s42",
    reset_num_timesteps = False,
)

new_model.save(SAVE_PATH)
print(f"[E1_pred SUBWP s42] Guardado en {SAVE_PATH}.zip")
