"""
Inferencia stage 4 — SUB-WP seed=42 — CICLO COMPLETO ENCADENADO.

Modelo: subwp_s123_stage4_final
100 episodios por goal × 28 goals = 2800 episodios.
Éxito = approach + exit completados en un único episodio encadenado.

Salida: inferencia_subwp/resultados/infer_subwp_s123_wp75_stage4.csv
"""
...
MODEL_PATH    = os.path.join(os.path.dirname(os.path.dirname(__file__)), "pruebas", "subwp_s123_wp75_stage4_final")
RUN_ID        = "subwp_s123_wp75"
STAGE         = 4
HEADING_SIGMA = 0.25
N_POR_GOAL    = 100
OUT_DIR       = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH      = os.path.join(OUT_DIR, "infer_subwp_s123_wp75_stage4.csv")

env   = WebotsEnv(map_path=MAP_PATH, stage=STAGE, heading_sigma=HEADING_SIGMA)
model = PPO.load(MODEL_PATH, env=env)

env.PROB_APPROACH_S4 = 1.0
env._max_steps = 4000   # margen extra para goals con paths largos

N_GOALS = len(env.goal_ids)
N_TOTAL = N_GOALS * N_POR_GOAL

print(f"\n{'='*60}")
print(f" Inferencia stage 4 SUB-WP — CICLO COMPLETO | {RUN_ID}")
...
print(f"\n{'='*60}")
print(f" RESUMEN — {N_TOTAL} ep deterministas | {RUN_ID} stage4")
print(f"{'='*60}")
