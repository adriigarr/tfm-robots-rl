"""
Inferencia dinámica — SUB-WP seed=42 — modelo stage6_din (reentrenado con peatones).

100 ep × 28 goals = 2800 ciclos.
World: warehouse_1_subwp.wbt (PEDESTRIAN_1 + PEDESTRIAN_2 activos).

Salida: inferencia_subwp/resultados/infer_din_retrained_s123.csv
"""
...
MODEL_PATH = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_s123_wp75_stage6_din_final")
RUN_ID     = "din_retrained_s123_subwp"
N_POR_GOAL = 100
OUT_DIR    = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH   = os.path.join(OUT_DIR, "infer_din_retrained_s123.csv")
...
print(f" World: warehouse_1_subwp.wbt (peatones activos)")
print(f"{'='*60}\n")
