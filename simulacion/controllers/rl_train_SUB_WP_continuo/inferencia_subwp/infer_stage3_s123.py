"""
Inferencia stage 3 — SUB-WP seed=123 — Exit puro, política determinista.

100 episodios por goal × 28 goals = 2800 episodios.
Salida: inferencia_subwp/resultados/infer_subwp_s123_wp75_stage3.csv
"""
...
MODEL_PATH    = os.path.join(os.path.dirname(os.path.dirname(__file__)), "pruebas", "subwp_s123_wp75_stage3_final")
RUN_ID        = "subwp_s123_wp75"
STAGE         = 3
HEADING_SIGMA = 0.25
N_POR_GOAL    = 100
OUT_DIR       = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH      = os.path.join(OUT_DIR, "infer_subwp_s123_wp75_stage3.csv")
...
print(f"\n{'='*60}")
print(f" Inferencia stage 3 SUB-WP | {RUN_ID} | {N_GOALS} × {N_POR_GOAL} = {N_TOTAL} ep")
print(f" Exit puro | heading_sigma={HEADING_SIGMA} rad")
print(f"{'='*60}\n")
...
for goal_idx in range(N_GOALS):
    ...
    for ep in range(1, N_POR_GOAL + 1):
        ...
        resultados.append({"episodio": ep_global, "goal_id": goal_id,
                            "tipo_episodio": "exit", "resultado": resultado,
...
print(f"\n{'='*60}")
print(f" RESUMEN — {N_TOTAL} ep | {RUN_ID} stage3")
