"""
Inferencia stage 4 r2 — SUB-WP seed=42 — Ciclo approach+exit | dropoff corregido.

Salida: inferencia_subwp/resultados/infer_subwp_s123_wp75_r2_stage4.csv
"""
...
MODEL_PATH = os.path.join(CONTROLLER_DIR, "pruebas", "subwp_s123_wp75_r2_stage4_final")
RUN_ID     = "subwp_s123_wp75_r2"
STAGE      = 4
N_POR_GOAL = 100
OUT_DIR    = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH   = os.path.join(OUT_DIR, "infer_subwp_s123_wp75_r2_stage4.csv")
...
print(f"\n{'='*60}")
print(f" Inferencia stage 4 r2 | {RUN_ID} | ciclo approach+exit")
print(f" {N_GOALS} goals × {N_POR_GOAL} ep = {N_TOTAL} episodios")
print(f"{'='*60}\n")

FIELDS = ["episodio", "goal_id", "resultado", "pasos", "reward", "llego_estanteria"]
stats  = {gid: {"exito": 0, "col_approach": 0, "col_exit": 0, "truncado": 0}
...
for goal_idx in range(N_GOALS):
    ...
    for ep in range(1, N_POR_GOAL + 1):
        ...
        llego = bool(info.get("llego_estanteria", False))
        if info.get("exito"):
            resultado = "exito";       stats[goal_id]["exito"]       += 1
        elif info.get("colision") and not llego:
            resultado = "col_approach"; stats[goal_id]["col_approach"] += 1
        elif info.get("colision"):
            resultado = "col_exit";    stats[goal_id]["col_exit"]    += 1
        else:
            resultado = "truncado";    stats[goal_id]["truncado"]    += 1

        with open(CSV_PATH, "a", newline="") as f:
            csv.DictWriter(f, fieldnames=FIELDS).writerow(
                {"episodio": ep_global, "goal_id": goal_id, "resultado": resultado,
                 "pasos": pasos, "reward": round(reward_ep, 2), "llego_estanteria": int(llego)})
...
print(f"\n{'='*60}")
print(f" RESUMEN — {N_TOTAL} ep | {RUN_ID} stage4 r2")
...
for gid, v in stats.items():
    ...
    print(f"    {gid}: {v['exito']}/{N_POR_GOAL} ({tasa:.0f}%){'  ⚠' if tasa < 60 else ''}")
print(f"{'='*60}\n[CSV] {CSV_PATH}")
