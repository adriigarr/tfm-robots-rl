"""
Inferencia v1 estático + REPLANIFICACIÓN — SUB-WP s42, peatones congelados.

Modelo: subwp_s42_wp75_stage5_final (v1, ped_obs=False, 40 dims).
Peatones: físicamente presentes pero congelados en cada step.
Replanning: activo.

Sin replanning: 15.5% (§23). Con replanning: ???
Aísla el efecto del replanning sin entrenamiento dinámico de por medio.

Salida: inferencia_subwp/resultados/infer_subwp_s42_stage5_replanning.csv
"""
...
RUN_ID     = "subwp_s42_static_replanning"
N_POR_GOAL = 100
OUT_DIR    = os.path.join(os.path.dirname(__file__), "resultados")
CSV_PATH   = os.path.join(OUT_DIR, "infer_subwp_s42_stage5_replanning.csv")
...
print(f"[INFO] {len(ped_nodes)} peatón(es) en el mundo (congelados)")
